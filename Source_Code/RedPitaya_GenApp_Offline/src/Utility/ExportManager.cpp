                                                   ...
                                                   const std::atomic<bool> &cancelExportFlag)
{
    ...
    if (cancelExportFlag.load())
        return false;
    if (!SSHManager::scp_transfer(hostname, password, tempModelDir, remoteModelDir, privateKeyPath))
        return false;
    ...
    if (fs::is_directory(src))
    {
        for (const auto &file : fs::directory_iterator(src))
        {
            ...
            if (!SSHManager::scp_transfer(hostname, password, file.path().string(), remoteFilePath, privateKeyPath))
                return false;
        }
    }
    else if (fs::is_regular_file(src))
    {
        if (cancelExportFlag.load())
            return false;
        if (!SSHManager::scp_transfer(hostname, password, src.string(), remoteVersionDir, privateKeyPath))
            return false;
    }
